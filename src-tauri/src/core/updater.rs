...
impl SilentUpdater {
    ...
    pub async fn try_install_on_startup(&self, app_handle: &tauri::AppHandle) -> bool {
        ...
        let updater_builder = {
            ...
            let lang_id = nsis_language_id(&ns_vpn_i18n::current_language(verge_lang.as_deref()));
...
impl SilentUpdater {
    ...
    async fn ask_user_to_install(app_handle: &tauri::AppHandle, version: &str) -> bool {
        ...
        let title = ns_vpn_i18n::t!("notifications.updateReady.title");
        let body = ns_vpn_i18n::t!("notifications.updateReady.body").replace("{version}", version);
        let install_now = ns_vpn_i18n::t!("notifications.updateReady.installNow").into_owned();
        let later = ns_vpn_i18n::t!("notifications.updateReady.later").into_owned();
