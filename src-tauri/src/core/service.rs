...
fn linux_running_as_root() -> bool {
    ...
    use tauri_plugin_ns_vpn_sysinfo::is_current_app_handle_admin;
...
fn uninstall_service() -> Result<()> {
    ...
    // ns_vpn_i18n::sync_locale(Config::verge().await.latest_arc().language.as_deref());

    let prompt = ns_vpn_i18n::t!("service.adminUninstallPrompt");
    // 先清理服务残留,再执行卸载器。
...
fn install_service() -> Result<()> {
    ...
    let gid = tauri_plugin_ns_vpn_sysinfo::current_gid();
    let prompt = ns_vpn_i18n::t!("service.adminInstallPrompt");
