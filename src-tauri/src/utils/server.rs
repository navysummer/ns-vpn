...
use ns_vpn_logging::{Type, logging, logging_error};
...
#[cfg(feature = "verge-dev")]
fn release_dev_quit_latch(quit_requested: &AtomicBool, outcome: ns_vpn_signal::ShutdownOutcome) {
    if matches!(outcome, ns_vpn_signal::ShutdownOutcome::Canceled) {
...
mod tests {
    ...
    async fn dev_quit_route_requires_private_auth_ready_state_and_dispatches_once() -> anyhow::Result<()> {
        ...
        release_dev_quit_latch(&REQUESTED, ns_vpn_signal::ShutdownOutcome::Canceled);
        assert!(!REQUESTED.load(Ordering::Acquire));
