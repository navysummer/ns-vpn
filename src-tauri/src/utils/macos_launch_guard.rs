...
pub fn enforce_before_initialization() -> LaunchDisposition {
    ns_vpn_i18n::sync_locale(None);

    let executable = match std::env::current_exe() {
        Ok(executable) => executable,
        Err(error) => {
            let message = ns_vpn_i18n::t!("launchGuard.currentExeError").replace("{error}", &error.to_string());
    ...
    let home = match current_user_home() {
        Ok(home) => home,
        Err(error) => {
            let message = ns_vpn_i18n::t!("launchGuard.homeDirError").replace("{error}", &error.to_string());
            show_message(&message);
            return LaunchDisposition::Exit;
        }
    };
    ...
    match evaluate_install_location(&executable, &home) {
        ...
        LaunchLocation::Translocated => {
            show_message(&ns_vpn_i18n::t!("launchGuard.translocated"));
            LaunchDisposition::Exit
        }
        LaunchLocation::Rejected { reason } => {
            let reason = match reason {
                LaunchRejectionReason::NotInApplicationBundle => {
                    ns_vpn_i18n::t!("launchGuard.notInBundle").into_owned()
                }
                LaunchRejectionReason::CanonicalizeBundleFailed { error } => {
                    ns_vpn_i18n::t!("launchGuard.canonicalizeBundleError").replace("{error}", &error)
                }
            };
            let message = ns_vpn_i18n::t!("launchGuard.rejected").replace("{reason}", &reason);
            show_message(&message);
            LaunchDisposition::Exit
        }
...
fn move_and_relaunch(bundle: &Path, home: &Path) -> LaunchDisposition {
    ...
    let Some(bundle_name) = bundle.file_name() else {
        show_message(&ns_vpn_i18n::t!("launchGuard.bundleNameError"));
        return LaunchDisposition::Exit;
    };
    ...
    let prompt = if exists {
        ns_vpn_i18n::t!("launchGuard.replacePrompt").replace("{destination}", &destination_display)
    } else {
        ns_vpn_i18n::t!("launchGuard.movePrompt").replace("{destination}", &destination_display)
    };
    if !confirm(&prompt) {
        return LaunchDisposition::Exit;
    }
    if let Err(error) = std::fs::create_dir_all(&destination_root) {
        let message =
            ns_vpn_i18n::t!("launchGuard.createApplicationsDirError").replace("{error}", &error.to_string());
        show_message(&message);
        return LaunchDisposition::Exit;
    }
    ...
    if !copy.is_ok_and(|status| status.success()) {
        let _ = remove_existing_target(&staging);
        show_message(&ns_vpn_i18n::t!("launchGuard.moveFailed"));
        return LaunchDisposition::Exit;
    }
    let backup = match activate_staged_bundle(&staging, &destination, &backup) {
        ...
        Err(error) => {
            let _ = remove_existing_target(&staging);
            let message = ns_vpn_i18n::t!("launchGuard.replaceFailed").replace("{error}", &error.to_string());
            show_message(&message);
            return LaunchDisposition::Exit;
        }
    };
    ...
    if !relaunched {
        ...
        show_message(&ns_vpn_i18n::t!("launchGuard.relaunchFailed"));
        return LaunchDisposition::Exit;
    }
...
fn confirm(message: &str) -> bool {
    let cancel_button = ns_vpn_i18n::t!("launchGuard.cancel");
    let move_button = ns_vpn_i18n::t!("launchGuard.move");
    let script = format!(
...
fn show_message(message: &str) {
    let ok_button = ns_vpn_i18n::t!("launchGuard.ok");
    let script = format!(
        "display dialog \"{}\" buttons {{\"{}\"}} default button \"{}\" with icon caution",
        escape_osascript(message),
