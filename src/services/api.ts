...
async function asyncRetry<T>(
  fn: (bail: (err: Error) => void) => Promise<T>,
  options: { retries?: number; minTimeout?: number; maxTimeout?: number; randomize?: boolean },
): Promise<T> {
  const { retries = 3, minTimeout = 1000, maxTimeout = 4000, randomize = false } = options
  let lastError: Error | undefined

  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      let bailed = false
      const bail = (err: Error) => { bailed = true; throw err }
      return await fn(bail)
    } catch (err) {
      if (err instanceof Error) lastError = err
      if (attempt === retries) break
      const baseDelay = Math.min(minTimeout * Math.pow(2, attempt), maxTimeout)
      const delay = randomize ? baseDelay * (0.5 + Math.random() * 0.5) : baseDelay
      await new Promise((resolve) => setTimeout(resolve, delay))
    }
  }

  throw lastError ?? new Error('asyncRetry failed')
}
