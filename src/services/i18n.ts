...
const STARTUP_LANGUAGE_SECTIONS = [
  'layout', 'home', 'shared', 'settings', 'profiles',
  'proxies', 'tests', 'rules', 'connections', 'logs',
] as const
...
const loadLanguageSections = async (language: string, sections: readonly string[]) => {
  try {
    const entries = await Promise.all(
      sections.map(async (section) => {
        const loader = localeLoaders[language]?.[section]
        if (!loader) {
          throw new Error(`Locale loader not found for language "${language}" section "${section}"`)
        }
    ...
    if (language !== FALLBACK_LANGUAGE) {
      console.warn(`Failed to load language ${language}, fallback to ${FALLBACK_LANGUAGE}, ${error}`)
...
const ensureLanguageSections = async (
  ...
  const missingSections = sectionList.filter((section) => !loadedSections.has(section))
...
export const changeLanguage = async (language: string) => {
  const targetLanguage = resolveLanguage(language)
  const loadedSections = getLoadedLanguageSections(i18n.language || FALLBACK_LANGUAGE)
