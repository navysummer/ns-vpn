<template>
  <div :style="{ display: 'flex', alignItems: 'center', gap: '4px', ...(sx || {}) }">
    <n-button quaternary circle size="small" :title="t('proxies.page.tooltips.locate')" @click="onLocation">
      <template #icon>
    ...
    <div v-if="textState === 'filter'" :style="{ marginLeft: '4px', flex: '1 1 auto' }">
      <BaseSearchBox
        :autoFocus="autoFocus"
        :value="filterText"
        :searchState="{ matchCase: filterMatchCase, matchWholeWord: filterMatchWholeWord, useRegularExpression: filterUseRegularExpression }"
        @search="(text: string, state: any) => onHeadState({ filterText: state.text, filterMatchCase: state.matchCase, filterMatchWholeWord: state.matchWholeWord, filterUseRegularExpression: state.useRegularExpression })"
      />
    </div>
    <div v-if="textState === 'url'" :style="{ marginLeft: '4px', flex: '1 1 auto' }">
      <n-input
        size="small"
        :value="testUrl"
...
onMounted(() => { setTimeout(() => { autoFocus.value = true }, 100) })
