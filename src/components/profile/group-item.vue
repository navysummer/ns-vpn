<template>
  ...
  >
    <img
      v-if="group.icon?.trim().startsWith('http')"
      :src="iconCachePath || group.icon"
      width="32"
      style="margin-right: 12px; border-radius: 6px;"
    />
    <img
      v-else-if="group.icon?.trim().startsWith('data')"
      :src="group.icon"
      width="32"
      style="margin-right: 12px; border-radius: 6px;"
    />
    <img
      v-else-if="group.icon?.trim().startsWith('<svg')"
    ...
    >
      ...
      <div style="display: flex; align-items: center; padding-top: 2px; color: #ccc;">
        <span
          :style="{
...
const iconCachePath = useIconCache({ icon: props.group.icon, cacheKey: props.group.name.replaceAll(' ', '') })
