...
export const useConnectionData = (options?: { enabled?: boolean }) => {
  ...
  const data = shallowRef<ConnectionMonitorData>(initConnData)

  onMounted(() => {
    if (!enabled) return
    const unsubscribe = subscribeConnectionData(() => {
      data.value = { ...connectionData }
    })
    data.value = { ...connectionData }
...
export const useConnectionSummaryData = (options?: { enabled?: boolean }) => {
  const enabled = options?.enabled ?? true
  const data = shallowRef<ConnectionSummaryData>(initConnSummaryData)

  onMounted(() => {
    if (!enabled) return
    const unsubscribe = subscribeConnectionSummary(() => {
      data.value = { ...connectionSummary }
    })
    data.value = { ...connectionSummary }
    onUnmounted(unsubscribe)
  })

  const refreshGetClashConnectionSummary = () => {
    refreshConnectionData()
  }
