...
export const useLogData = () => {
  ...
  const hasLoadedInitialLogsRef = shallowRef(false)
  ...
  >({
    ...
    setupHandlers: ({ next, scheduleReconnect, isMounted }) => {
      ...
      return {
        ...
        async onConnected() {
          if (hasLoadedInitialLogsRef.value) {
            return
          }
          const logs = await getClashLogs()
          hasLoadedInitialLogsRef.value = true
          if (isMounted()) {
            next(null, (current) => {
  ...
  onMounted(() => {
    if (!logLevel) {
      previousLogLevelRef.value = logLevel ?? undefined
      return
    }

    if (previousLogLevelRef.value === logLevel) {
      return
    }

    previousLogLevelRef.value = logLevel
    hasLoadedInitialLogsRef.value = false
    refresh()
  }, [logLevel, refresh])
