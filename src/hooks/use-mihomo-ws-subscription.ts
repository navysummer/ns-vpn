...
interface SharedSubscriptionEntry {
  ...
  connecting: boolean
  refHolders: Set<{ value: MihomoWebSocket | null }>
...
export const useMihomoWsSubscription = <T>(
  ...
  const date = ref(initialSubscriptionDate)
  const setDate = (val: number) => { date.value = val }
  const lsVal = typeof localStorage !== 'undefined' ? localStorage.getItem(storageKey) : null
  if (lsVal !== null) {
    const parsed = Number(lsVal)
    if (!isNaN(parsed)) date.value = parsed
  }

  const subscriptKey = buildSubscriptKey(date.value)
  const subscriptionCacheKey = subscriptKey ? `$sub$${subscriptKey}` : null
  const lastSubscriptionCacheKeyRef = ref<string | null>(null)
  if (subscriptionCacheKey) {
    lastSubscriptionCacheKeyRef.value = subscriptionCacheKey
  }
  const responseCacheKey =
    subscriptionCacheKey ?? lastSubscriptionCacheKeyRef.value

  const wsRef = shallowRef<MihomoWebSocket | null>(null)
